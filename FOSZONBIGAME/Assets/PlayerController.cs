...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (m_controlType == ControlType.Move)
        {
            ...
            Vector3 body = Vector3.Scale(Camera.main.transform.forward, new Vector3(1, 0, 1)).normalized;
            if(body!=Vector3.zero)
            {
                this.transform.rotation = Quaternion.LookRotation(body);
            }
            if (dir == Vector3.zero)
            {
            ...
            else
            {
                ...
                Debug.Log(dir);// メインカメラを基準に入力方向のベクトルを変換する
